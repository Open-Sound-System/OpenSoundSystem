...
for n in `ls $OSSLIBDIR/modules|egrep -v "osscore|softoss|vmix|imux"`
do
  modadmin -U $n > /dev/null 2>&1
done

for MOD in osscore softoss vmix imux
do
  modadmin -U $MOD > /dev/null 2>&1
done

# (re)install osscore and vmix

rm -f $OSSLIBDIR/modules/*/install.log

for MOD in osscore vmix imux
do
