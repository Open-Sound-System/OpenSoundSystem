...
static irqreturn_t
osscore_intr (int irq, void *arg, struct pt_regs *regs)
{
  int serviced;
			 ...
			 oss_bottomhalf_handler_t bottom)
{
  ...
  if ((err =
       request_irq (pci_irq_line, osscore_intr, SA_SHIRQ, name, intr)) < 0)
    {
...
EXPORT_SYMBOL (oss_audio_set_devname);
...
EXPORT_FUNC (midiparser_input_buf);
